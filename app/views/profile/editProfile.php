...
<div class="min-h-screen text-white p-6 flex justify-center py-20">
  <div class="max-w-5xl w-full bg-gray-800 shadow-lg rounded-lg p-8 space-y-8">

    <!-- Profile Picture & Username Section -->
    <div class="flex items-center space-x-6 mb-8">
      <div class="relative">
        <img class="w-32 h-32 rounded-full border-4 border-gray-700" src="<?= '/researchhub_project/public/uploads/profile_pics/' . htmlspecialchars($user_data[0]['profile_picture']) ?>" alt="Profile Picture">
        <label for="profilePic" class="absolute bottom-0 right-0 bg-gray-700 hover:bg-gray-600 cursor-pointer p-2 rounded-full">
      ...
      <div>
        ...
        <form action="../../controllers/ProfileController.php" method="POST" enctype="multipart/form-data" class="mt-1">
          <input type="file" id="profilePic" class="hidden" name="profilePic" required>
          <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
          <button type="submit" name="updateProfilePic" class="px-4 py-1.5 text-sm bg-gradient-to-r from-teal-400 to-green-500 text-white rounded-md">Save Profile Picture</button>
        </form>
      </div>
    </div>
    <a href="verify_email.php" id="editBioBtn" class="px-4 py-2 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-teal-500 hover:to-blue-500 text-white rounded-lg hover:bg-gray-500">Verify Email address</a>
    <!-- Bio Section -->
    <div class="bg-gray-700 p-4 rounded-lg shadow-lg">
      <h3 class="text-xl font-semibold mb-2">Bio</h3>
      <p id="bioDisplay" class="text-gray-300"><?php echo htmlspecialchars($user_data[0]['bio']) ?></p>
      <button id="editBioBtn" class="mt-3 px-4 py-2 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-teal-500 hover:to-blue-500 text-white rounded-lg hover:bg-gray-500">Edit Bio</button>

      <!-- Bio Edit Form -->
      <form action="../../controllers/ProfileController.php" method="POST" id="bioForm" class="hidden mt-4">
        <textarea id="bioInput" name="bio" rows="4" class="w-full p-3 border border-gray-600 rounded-md bg-gray-900 text-white" placeholder="Tell us about yourself..."></textarea>
    ...
    <div class="bg-gray-700 p-4 rounded-lg shadow-lg">
      ...
      <ul id="affiliationsList" class="text-gray-300 space-y-2">
      ...
      <button id="editAffiliationsBtn" class="mt-3 px-4 py-2 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-teal-500 hover:to-blue-500 text-white rounded-lg hover:bg-gray-500">Edit Affiliations</button>
      ...
      <div id="affiliationsForm" class="hidden mt-4">
        <div class="space-y-4">
          ...
          <!-- Add New Affiliation -->
          <button id="addAffiliationBtn" class="px-4 py-2 bg-gradient-to-r from-teal-400 to-green-500 text-white rounded-lg">Add New Affiliation</button>
        </div>

        <form action="../../controllers/ProfileController.php" method="POST" id="affiliationForm" class="hidden mt-4">
          <input type="text" name="institution_name" placeholder="Institution Name" class="mb-2 p-2 border border-gray-600 rounded-md  bg-gray-900 text-white w-full">
          <input type="text" name="department" placeholder="Department" class="mb-2 p-2 border border-gray-600 rounded-md  bg-gray-900 text-white w-full">
          <input type="text" name="position" placeholder="Position" class="mb-2 p-2 border border-gray-600 rounded-md  bg-gray-900 text-white w-full">
          <div class="flex space-x-2">
            <input type="date" name="start_date" class="p-2 border border-gray-600 rounded-md bg-gray-900 text-white">
            <input type="date" name="end_date" class="p-2 border border-gray-600 rounded-md bg-gray-900 text-white">
          </div>
          <div class="flex justify-end space-x-4 mt-3">
    ...
    <div class="bg-gray-700 p-4 rounded-lg shadow-lg">
      ...
      <div id="interestsList" class="flex flex-wrap gap-3">
        <?php foreach ($interests as $interest): ?>
          <span class="px-3 py-1 bg-gray-600 text-sm rounded-lg shadow"><?php echo htmlspecialchars($interest['interest']); ?></span>
        <?php endforeach; ?>
      </div>
      <button id="editInterestsBtn" class="mt-3 px-4 py-2 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-teal-500 hover:to-blue-500 text-white rounded-lg hover:bg-gray-500">Edit Interests</button>
      ...
      <div id="interestsForm" class="hidden mt-4">
        ...
        <!-- Add New Interest -->
        <div class="mt-4">
          <button id="addInterestBtn" class="px-4 py-2 bg-gradient-to-r from-teal-400 to-green-500 rounded-lg text-white hover:bg-green-500">Add New Interest</button>
        </div>



        <form id="interestForm" class="hidden mt-4" action="../../controllers/ProfileController.php" method="POST">
          <input type="text" name="interest" placeholder="New Interest" class="mb-2 p-2 border border-gray-600 rounded-md bg-gray-900 text-white w-full">
